...
class Double_DQN(object):
    def __init__(self, env):
        ...
        self.__alternate_model = self.__model
    ...
    def __replay(self):
        ...
        current_act_list = self.__model.predict_on_batch(states)
        future_act_list = self.__model.predict_on_batch(next_states)
        future_act_list_alt = self.__alternate_model.predict_on_batch(next_states)

        for index, (state, action, reward, _, id, done) in enumerate(minibatch):
            max_future_q = reward + self.__gamma * future_act_list_alt[index][np.argmax(future_act_list[index])] * (1 - done)
            current_act = current_act_list[index]
            current_act[action] = (1 - self.__learning_rate) * current_act[action] + self.__learning_rate * max_future_q

            inputs.append(np.append(state, id, axis=0))
            targets.append(current_act)

        self.__model.fit(np.array(inputs), np.array(targets), batch_size=self.__batch_size, verbose=0, shuffle=True)
