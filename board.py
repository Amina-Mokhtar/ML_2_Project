...
class Board(object):
   ...
   draw < 0 : do not draw
   draw = 0 : draw in real time
   draw > 0 : draw with wait time draw
   '''
   def __init__(self, env, width, height,draw=-1):
      self.__draw = draw
      if self.__draw >= 0:
         pg.init()
         self.__env = env
   ...
   def draw_board(self,text=['']):
      if self.__draw >= 0:
         ...
         for t in range(len(text)):
            img = self.__font.render(text[t], True, Colors.WHITE)
            self.__screen.blit(img, (20, 20+t*20))
         self.__drawPieces() # draw pieces on board
         pg.display.update()
         for event in pg.event.get():
            if event.type == pg.QUIT:
               pg.quit()
         time.sleep(self.__draw)
   ...
   def __drawPieces(self):   # draw pieces on board from piece and obst vectors
